...
export default ({ ffmpeg,setScreen }) => {
    ...
    const clip = async () => {
        if (invideo && start.current && end.current) {
            ...
            const executed = await ffmpeg.exec(['-ss', `${startTime}`, '-to', `${endTIme}`, '-i', invideo.name, '-c', 'copy', `output_${invideo.name}` ])
            if(executed != 0){
                ...
                tab ?
                    invideo ?
                        <div >
                            ...
                            {invideo ? <button disabled={processing} onClick={clip} className="disabled:bg-gray-500 disabled:text-white active:scale-90 block m-auto mt-[50px] font-bold bg-dark_success text-md text-dark_bg py-[10px] px-[20px] rounded-md">Clip</button> : null}
                        </div>
