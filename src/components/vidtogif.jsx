...
export default ({ ffmpeg,setScreen }) => {
    ...
    const generateGif = async () => {
        if (invideo && start.current && duration.current) {
            const startTime = parseInt(start.current.value)
            const durationTime = parseInt(duration.current.value)
            
            if(durationTime > 15 || durationTime < 0){
                toast.error('Duration should not be greater than 15 sec and  be positive')
                return
            }
            ...
            if(await ffmpeg.exec(['-ss', `${startTime}`, '-t', `${durationTime}`, '-i', invideo.name, '-filter_complex', '[0:v] palettegen', 'palette.png']) != 0){
                toast.error('Something went wronge')
                return
            }
            const executed = await ffmpeg.exec(['-ss', `${startTime}`, '-t', `${durationTime}`, '-i', invideo.name, '-i', 'palette.png', '-filter_complex', '[0:v][1:v] paletteuse', 'output.gif'])
            if(executed != 0){
                toast.error('Something went wronge')
                return
            }else if (executed == 0){
            ...
            setOutput(URL.createObjectURL(new Blob([output.buffer])))
            setProcessing(false)
        }
                    ...
                    invideo ?
                        <div >
                            <div className="-z- mt-[40px] relative">
                                <video ref={video} className="bg-white" src={URL.createObjectURL(invideo)} controls></video>
                            ...
                            <div className="flex flex-col">
                                ...
                                <Inputfield fileEle={video} title="Duration" reference={duration} initial={2} pin={false}/>
                            </div>

                        
                            {invideo ? <button disabled={processing} onClick={generateGif} className="disabled:bg-gray-500 disabled:text-white active:scale-90 block m-auto mt-[50px] font-bold bg-dark_success text-md text-dark_bg py-[10px] px-[20px] rounded-md">Convert To Gif</button> : null}
                        </div>
...

                        {
                            output ?
                                <div className="my-[100px]">
                                    <img src={output} alt="" />
                                    <a className="active:scale-90 w-fit block m-auto mt-[20px] font-bold bg-dark_success text-md text-dark_bg py-[10px] px-[20px] rounded-md" href={output} download="output.gif">Download</a>
                                </div>
                                : null
                        }
