...
export default ({ ffmpeg,setScreen }) => {
    ...
    const clip = async () => {
        if (invideo && quality.current ) {
            const qualityVal = parseInt(quality.current.value)
            ...
            const executed = await ffmpeg.exec([ '-i', invideo.name, '-vf', `scale=-2:${qualityVal}`, `output_${invideo.name}`])
            if(executed != 0){
            ...
            const output = await ffmpeg.readFile(`output_${invideo.name}`)
                    ...
                    invideo ?
                        <div >
                            ...
                            <div className="flex mt-[40px] justify-center items-center">
                                <SelectField reference={quality} />
                            </div>
                            {invideo ? <button disabled={processing} onClick={clip} className="disabled:bg-gray-500 disabled:text-white active:scale-90 block m-auto mt-[50px] font-bold bg-dark_success text-md text-dark_bg py-[10px] px-[20px] rounded-md">Process Video</button> : null}
                        </div>
...

                        {
                            output ?
                                <div className="my-[100px] flex justify-center flex-col">
                                    ...
                                    <a className="active:scale-90 w-fit block m-auto mt-[20px] font-bold bg-dark_success text-md text-dark_bg py-[10px] px-[20px] rounded-md" href={output} download={`output_${invideo.name}`}>Download</a>
                                </div>
                                : null
                        }
